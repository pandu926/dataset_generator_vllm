"""
Full RAG-Augmented LLM-as-Judge Evaluation Script
Compare base model vs fine-tuned model WITH RAG context for both
5-Point Likert Scale (Research Best Practices: G-Eval, MT-Bench)
Optimized for A100 80GB with vLLM batch processing

KEY DIFFERENCE from evaluate_llm_judge.py:
- BOTH base and fine-tuned models receive RAG context during generation
- LLM-as-Judge also receives RAG context for fact-verification
- This provides a FAIR comparison with same information access
...
# Add parent directory to path for E5 embedding import
...
class EvalConfig:
    ...
    # Paths
    test_dataset_path: str = "../data/split/multiturn_dataset_cleaned_no_thought_test.json"
    output_path: str = "./outputs/llm_judge_rag_evaluation_results.json"
    chunks_path: str = "../data/chunks/chunks.jsonl"
    
    # RAG Settings
    rag_top_k: int = 3
    use_rag_for_generation: bool = True  # NEW: RAG for model generation
...
# =============================================================================
# RAG-AUGMENTED GENERATION PROMPT
# =============================================================================
...
"""
    ...
    def _parse_scores(self, text: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Validate required fields - 6 criteria
...
# =============================================================================
# MODEL RESPONSE GENERATORS (RAG-AUGMENTED)
# =============================================================================
...
def generate_responses_batch_with_rag(
    ...
    # Format prompts with RAG context for Gemma
    formatted_prompts = []
    ...
    inputs = tokenizer(
        ...
        padding=True,
        truncation=True,
        max_length=2048,  # Increased for RAG context
    ).to(model.device)
...
def generate_all_responses_with_rag(
    ...
    **gen_kwargs
    ...
    for i in tqdm(range(num_batches), desc="Generating responses (with RAG)"):
...
def evaluate(config: EvalConfig = None):
    """Main evaluation function comparing base vs fine-tuned model WITH RAG."""
    ...
    print("\n" + "="*70)
    print("FULL RAG-AUGMENTED LLM-AS-JUDGE EVALUATION")
    print("Base Model + RAG vs Fine-tuned Model + RAG")
    print("="*70)
    ...
    print(f"RAG for Generation: {config.use_rag_for_generation}")
    print("="*70)
    ...
    results = {
        "config": {
            ...
            "rag_for_generation": config.use_rag_for_generation,
    ...
    # ===========================================
    # LOAD RAG SYSTEM FIRST (Before model generation)
    # ===========================================
    print("\n" + "-"*40)
    print("STEP 0: Loading RAG Knowledge Base")
    print("-"*40)
    
    chunks = []
    embed_service = None
    contexts = [""] * len(prompts)  # Default empty contexts
    ...
    if HAS_RAG and config.use_rag_for_generation:
        ...
        if chunks:
            ...
            # Retrieve contexts for all prompts ONCE
            ...
            print("Warning: No chunks loaded, proceeding without RAG")
    else:
        print("RAG for generation disabled or not available")
    
    # ===========================================
    # Generate from BASE MODEL with RAG
    # ===========================================
    print("\n" + "-"*40)
    print("STEP 1: Generating responses from BASE MODEL + RAG")
    print("-"*40)
    
    base_model, base_tokenizer = load_base_model(config.base_model_name)
    
    base_responses = generate_all_responses_with_rag(
        base_model, base_tokenizer, test_samples, contexts,
    ...
    # ===========================================
    # Generate from FINETUNED MODEL with RAG
    # ===========================================
    print("\n" + "-"*40)
    print("STEP 2: Generating responses from FINETUNED MODEL + RAG")
    print("-"*40)
    ...
    ft_responses = generate_all_responses_with_rag(
    ...
    # Detailed results - include category, RAG context, and all responses
    for i, (sample, base_resp, ft_resp, base_sc, ft_sc, ctx) in enumerate(
        zip(test_samples, base_responses, ft_responses, base_scores, ft_scores, contexts)
    ):
        results["detailed_results"].append({
            "id": sample["id"],
            "category": sample.get("category", ""),
            "prompt": sample["prompt"],
            "rag_context_used": ctx[:500] + "..." if len(ctx) > 500 else ctx,  # Truncate for output
    ...
    # ===========================================
    print("\n" + "="*75)
    print("RAG-AUGMENTED EVALUATION RESULTS - 6-POINT CRITERIA")
    print("BOTH models received RAG context during generation")
    print("="*75)
    print(f"\n{'Dimension':<20} {'Base+RAG':>12} {'FT+RAG':>12} {'Improvement':>12}")
    print("-"*75)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Full RAG-Augmented LLM-as-Judge Evaluation")
    ...
    parser.add_argument("--output", type=str, 
                        default="./outputs/llm_judge_rag_evaluation_results.json",
    ...
    parser.add_argument("--no_rag_generation", action="store_true",
                        help="Disable RAG for model generation (only use for judge)")
    ...
    config = EvalConfig(
        ...
        use_rag_for_generation=not args.no_rag_generation,
    )
